...
export class AppController {
  ...
  @Get('user/:id/async')
  getUserAsync(@Param('id') id: string): Observable<User> {
  ...
  deleteUserAsync(@Param('id') id: string): Observable<void> {
    return this.inMemoryDBService.deleteAsync(id);
  }

  @Delete('users')
  deleteUsers(@Body() ids: string[]): void {
    return this.inMemoryDBService.deleteMany(ids);
  }

  @Delete('users/async')
  deleteUsersAsync(@Body() ids: string[]): Observable<void> {
